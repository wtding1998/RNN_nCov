...
def train(command=False):
    if command == True:
        ...
        p.add('--data_normalize', type=str, help='data_normalize method', default='x')
        ...
        # -- model
        p.add('--model', type=str, help='STNN Model', default='v1')
    ...
    params = [{'params': model.factors_parameters(), 'weight_decay': opt.wd_z},
            {'params': model.dynamic.parameters(), 'weight_decay': opt.wd_z},
            {'params': model.decoder.parameters(), 'weight_decay': opt.wd_z}]
    ...
    for e in pb:
        ...
        if opt.test:
            ...
            if command:
                pb.set_postfix(loss=logs_train['loss'], test=opt.sum_score)
            else:
                print(e, 'loss=', logs_train['loss'], 'test=', opt.sum_score)
